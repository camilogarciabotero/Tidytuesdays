...
```{r exploratoryanimated, warning=FALSE, message=FALSE, fig.height = 10, fig.width = 10, dpi= 300, include=FALSE, eval=FALSE}
(
movies_country_years <- netflix_titles %>% 
  select(country, release_year, type) %>% 
  filter(!is.na(country), type == "Movie", !is.na(release_year)) %>% 
  separate_rows(country, sep = ", ") %>% 
  count(country, sort = TRUE, release_year) %>% 
  filter(country != "Other") %>% 
  group_by(country) %>% 
  arrange(release_year) %>%
  summarize(release_year, cumsum = cumsum(n), n) %>%
  mutate(ordering = rank(cumsum), release_year = as.integer(release_year)) %>% 
  ungroup() 
  # pivot_wider(names_from = release_year, values_from = cumsum) %>% 
  # mutate(across(everything(), ~replace_na(.x, 0))) %>% 
  # pivot_longer(!c(country, n, ordering), names_to = "release_year",  values_to = "cumsum") %>% 
...
movies_country_year_animated <- ggplot(movies_country_years, aes(ordering, group = country)) +
  geom_tile(aes(y = cumsum/2, height = cumsum, width = 0.9), alpha = 0.5, fill = "red") +
  geom_text(aes(y = 0, label = country), hjust = 1.1, color = "white") +
  geom_text(aes(y = cumsum, label = cumsum), size = 3, color = "white", hjust = -1) +
  ...
  transition_time(movies_country_years$release_year) +
  ease_aes('linear')

animate(movies_country_year_animated, nframes = 500, fps = 10, width = 800, height = 800, end_pause = 25)
